...
class PharmaciesOnCallDAL {
    ...
    static async getPharmaciesOnCall() {
        try {
            const rows = await query('SELECT pharmacies.id, pharmacies.name, pharmacies.address, pharmacies.phone FROM pharmacies')
            return rows
        } catch (err) {
            throw err
        }
    ...
    static async addPharmaciesOnCall(req) {
        try  {
            const rows = await query('INSERT INTO pharmacies (name, address, phone) VALUES (?, ?, ?)',
    ...
    static async addSchedulesForPharmaciesOnCall(req) {
        try {
            const rows = await query('INSERT INTO pharmacies_schedules (pharmacy_id, date, start_time, end_time) VALUES (?, ?, ?, ?)', [req.body.pharmacy_id, req.body.date, req.body.start_time, req.body.end_time])
            return rows
        } catch (err) {
            throw err
        }
